...
int main(void)
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        prog["view"] = glm::lookAt(glm::vec3{0, 0, 10}, glm::vec3{0, 0, 0}, glm::vec3{0, 1, 0});
        ...
        prog.use();
        prog["blinn_shading"] = static_cast<int>(blinn_shading);
        prog["per_vertex"] = static_cast<int>(per_vertex);
        ...
        // 2. Show a simple window that we create ourselves. We use a Begin/End pair to created a named window.
        {
            ...
            ImGui::Checkbox("Per-vertex Lighting", &per_vertex);
            ImGui::Checkbox("Blinn-Phong Shading", &blinn_shading);
            ImGui::Checkbox("Flat Shading", &flat_shading);

            if(ImGui::Button("Flat Shader")) {
                auto new_mesh = StaticMesh::LoadMesh("../resource/sphere.obj", true);
                mesh.release();
                mesh = new_mesh;
            }
            ImGui::SameLine();
            if(ImGui::Button("Smooth Shader")) {
                auto new_mesh = StaticMesh::LoadMesh("../resource/sphere.obj", false);
                mesh.release();
                mesh = new_mesh;
            }
